...
int main() {
    // [gnu::file file example]

    auto file = gnu::file::File("file.cpp");
...
*/
    ...
    auto work  = gnu::file::work_dir();
    auto files = gnu::file::read_dir(work.path() + "/src");
    auto all   = gnu::file::Buf();

    for (auto& file : files) {
        if (file.is_file() == true) {
            auto buf = gnu::file::read(file.filename());
            auto sum = (uint64_t) 0;
